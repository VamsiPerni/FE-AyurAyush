...
const SignupPage = () => {
  ...
  const handleCheckAndSendOtp = async () => {
    ...
    try {
      ...
      showErrorToast(
        err.response?.data?.message || `Unable to send OTP: ${err.message}`,
      );
    } finally {
      setSendingOTP(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showErrorToast(
        err.response?.data?.message || `Unable to signup: ${err.message}`,
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-8">
      <Card className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Join AyurAyush today</p>
        </div>
