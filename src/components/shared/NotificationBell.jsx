...
const getNotifId = (n) =>
    `${n.appointmentId}-${n.type}-${new Date(n.timestamp).getTime()}`;
...
const NotificationBell = () => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            let res;
            if (role === "patient")
                res = await notificationService.getPatientNotifications();
            else if (role === "doctor")
                res = await notificationService.getDoctorNotifications();
            else if (role === "admin" || role === "sub_admin")
                res = await notificationService.getAdminNotifications();
            else return;
                  ...
                  <div className="relative w-full max-w-md bg-white dark:bg-dark-card h-full flex flex-col shadow-2xl animate-slide-in-right">
                      ...
                      <div className="flex-1 overflow-y-auto scrollbar-thin">
                          {loading && notifications.length === 0 ? (
                              ...
                              <div className="flex flex-col items-center justify-center h-full gap-3 px-8 text-center">
                                  ...
                                  <p className="text-xs text-neutral-400 dark:text-neutral-500">
                                      Activity from the last 30 days will appear
                                      here.
                                  </p>
                              </div>
                          ) : (
                              <div className="divide-y divide-neutral-50 dark:divide-dark-border">
                                  {notifications.map((n, idx) => {
                                      const cfg =
                                          typeConfig[n.type] || typeConfig.info;
                                      const Icon = cfg.icon;
                      ...
                      <div className="px-5 py-3 border-t border-neutral-100 dark:border-dark-border bg-neutral-50/50 dark:bg-dark-elevated/30 shrink-0">
                          <p className="text-xs text-neutral-400 dark:text-neutral-500 text-center">
                              Showing up to 50 notifications from the last 30
                              days
                          </p>
                      </div>
                  </div>
