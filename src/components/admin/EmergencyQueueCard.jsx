...
import { Clock, Calendar, PencilLine, ShieldAlert, X } from "lucide-react";

const EmergencyQueueCard = ({ appointment, onEditApprove, onReject }) => {
    const {
        _id,
        appointmentId,
        patient,
        ...
        <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-700/40 rounded-xl p-5 shadow-sm relative overflow-hidden flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-red-200/50 justify-end">
                ...
                <Button
                    variant="danger"
                    icon={PencilLine}
                    onClick={() => onEditApprove(appointment)}
