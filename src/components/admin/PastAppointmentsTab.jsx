...
const PastAppointmentsTab = () => {
    ...
    const loadData = useCallback(
        async ({ silent = false, targetPage = 1 } = {}) => {
            try {
    ...
    const handleNoShowSubmit = async () => {
        ...
        try {
            ...
            const res = await adminService.markNoShow(
            ...
            <CardHeader className="bg-neutral-50/80 border-b border-neutral-100 pb-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <CardTitle className="text-neutral-800 tracking-tight">
                                Past Appointments
                            </CardTitle>
                            <p className="text-xs text-neutral-500 mt-0.5">
                                All confirmed appointments whose date has passed
                            </p>
                        </div>
                    </div>
                    <Button
                        ...
                        size="sm"
                        icon={RefreshCw}
                        onClick={() => loadData({ targetPage: page })}
                    >
                        Refresh
                    </Button>
                </div>
                ...
                <div className="flex gap-1 mt-4 border-b border-neutral-200">
                    {SUB_TABS.map((t) => (
                        ...
                        >
                            <t.icon
                                className={`w-4 h-4 ${subTab === t.key ? "text-primary-600" : t.color}`}
                            />
                            {t.label}
                            <span
                        ...
                        <table className="min-w-full text-sm">
                            <thead>
                                <tr className="bg-neutral-50/60 border-b border-neutral-100 text-left">
                                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 uppercase tracking-wide">
                                        Patient
                                    </th>
                                    <th className="px-5 py-3 text-xs font-semibold text-neutral-600 uppercase tracking-wide">
                                        Doctor
                            ...
                            <tbody>
                                {displayList.map((apt) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-5 py-3 text-neutral-700 font-semibold">
                                            {formatDate(apt.date)}
                                        </td>
                                        <td className="px-5 py-3 text-neutral-600">
                                            {apt.timeSlot}
                    ...
                    {refundTarget && (
                        <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-3 text-sm">
                            <p className="font-semibold text-neutral-800">
                                {refundTarget.patientName}
                            </p>
                            <p className="text-neutral-500">
                                Dr. {refundTarget.doctorName} &bull;{" "}
                                {formatDate(refundTarget.date)} &bull;{" "}
                                {refundTarget.timeSlot}
                            </p>
                            <p className="text-success-700 font-semibold mt-1">
                                Paid ₹{refundTarget.payment?.amount} &mdash;
                                full refund will be initiated
                            </p>
                        </div>
                    )}
                    <p className="text-sm text-neutral-600">
                        This will initiate a full refund to the patient. This
                ...
                <div className="space-y-4">
                    {noShowTarget && (
                        <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-3 text-sm">
                            ...
                            <p className="text-neutral-500">
                                Dr. {noShowTarget.doctorName} &bull;{" "}
                                {formatDate(noShowTarget.date)} &bull;{" "}
                                {noShowTarget.timeSlot}
                            </p>
                            {noShowTarget.payment && (
                                <p className="text-success-700 font-semibold mt-1">
                                    Paid ₹{noShowTarget.payment.amount} — refund
                                    will be initiated
                                </p>
                            )}
                        </div>
